...
const uploadImages = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const findGarage = await Garage.findByIdAndUpdate(
      id,
      {
        logo: urls[0].url,
      },
...
const createGarage = asyncHandler(async (req, res) => {
  ...
  if (!findCoordonnee) {
    const { name, intervention_address, logo, observation, price, tire_size } =
      req.body;
    const infosGarage = {
      name,
...
const coordonneeGarages = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    let fields = "-__v";
    let allCoordonnee = Coordonnee.find(objQuery)
      .where("role")
      .equals("garage");
    if (req.query?.fields) {
